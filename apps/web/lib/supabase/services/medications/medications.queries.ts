  ...
  try {
    ...
    const medicamentosActivos = reminders.map((reminder) => {
      ...
      for (const hora of horarios) {
        ...
        horaDate.setHours(parseInt(hours || '0'), parseInt(minutes || '0'), 0, 0);
    ...
    const summary: ActiveMedicationsSummary = {
      ...
      total_recordatorios: reminders.reduce((sum: number, r) => sum + (r.horarios as string[]).length, 0),
      proxima_toma: proximaToma, // Now compatible
